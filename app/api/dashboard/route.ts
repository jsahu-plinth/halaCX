...
export async function GET() {
  ...
  if (session.preview) return NextResponse.json({ preview: true, user: session, calls: previewCalls, knowledge: previewKnowledge, phoneNumbers: [], agent: { name: "Maya", status: "ready", languages: ["English", "Arabic", "Hindi"] } });

  const [calls, knowledge, agents, workspaces, phoneNumbers] = await Promise.all([
    ...
    query("select id,phone_number,friendly_name,status,inbound_enabled,scenario,voice_provider,agent_id,last_error,updated_at from phone_numbers where workspace_id=$1 order by created_at", [session.workspaceId]),
  ]);
  if (!workspaces.rows[0]) return NextResponse.json({ error: "Workspace not found" }, { status: 404 });
  return NextResponse.json({ preview: false, user: session, workspace: workspaces.rows[0], calls: calls.rows, knowledge: knowledge.rows, phoneNumbers: phoneNumbers.rows, agent: agents.rows[0] });
}
