...
const voiceProviders = [
  { key: "openai", name: "Coral", tone: "Balanced", detail: "Quick, clear and naturally conversational", badge: "LOWEST LATENCY" },
  { key: "sarvam", name: "Priya", tone: "Warm", detail: "Natural Indian English and code-mixed speech", badge: "INDIA NATIVE" },
  { key: "cartesia", name: "Skylar", tone: "Expressive", detail: "Bright, lively global English delivery", badge: "MORE EXPRESSION" },
] as const;
...
export default function MarketingScenes() {
  ...
  function changePhoneNumber(value: string) {
    const countryCode = phone.startsWith("+91") && !phone.startsWith("+971") ? "+91" : "+971";
    setPhone(countryCode + value.replace(/\D/g, ""));
  }
  ...
  return <>
    ...
    <section className="live-demo-lab" id="live-demo">
      <div className="demo-form-side"><p>LIVE DEMO</p><h2>Don’t take our word for it.<br/>Take the call.</h2><span>Choose a conversation, choose a voice, then enter the number you want us to call.</span><div className="context-tabs" role="tablist">{contexts.map(item=><button type="button" key={item.key} className={context.key===item.key?"active":""} onClick={()=>setContext(item)} role="tab" aria-selected={context.key===item.key}>{item.label}</button>)}</div><p className="context-line">{context.line}</p><form onSubmit={requestCall}><label htmlFor="demo-phone">YOUR MOBILE NUMBER</label><div><select aria-label="Country code" value={phone.startsWith("+91")&&!phone.startsWith("+971")?"+91":"+971"} onChange={event=>changePhoneCountry(event.target.value)}>{phoneCountries.map(country=><option key={country.code} value={country.code}>{country.label}</option>)}</select><input id="demo-phone" value={phone.replace(/^\+(?:971|91)/, "")} onChange={event=>changePhoneNumber(event.target.value)} inputMode="numeric" aria-label="Mobile number without country code" placeholder="Mobile number"/><button disabled={callState==="loading"}><Phone weight="fill"/>{callState==="loading"?"Calling…":"Call me"}</button></div>{message&&<small className={callState}>{message}</small>}</form></div>
      <div className="demo-storyboard"><div className="provider-picker"><header><label>CHOOSE THE VOICE</label><span>03 PERSONALITIES</span></header><div role="radiogroup" aria-label="Voice personality">{voiceProviders.map(provider=><button type="button" key={provider.key} data-provider={provider.key} className={voiceProvider===provider.key?"active":""} onClick={()=>setVoiceProvider(provider.key)} role="radio" aria-checked={voiceProvider===provider.key}><span className="provider-art" aria-hidden="true"><i className="provider-orbit"/><i className="provider-mark">{Array.from({length:5}).map((_,index)=><b key={index}/>)}</i><em>{provider.name.slice(0,1)}</em></span><span className="provider-copy"><i>{provider.badge}</i><strong>{provider.name}</strong><small>{provider.detail}</small><span><b>{provider.tone}</b><em>Select voice →</em></span></span></button>)}</div></div><div className="demo-flow"><p>WHAT HAPPENS NEXT</p><ol><li><span>1</span><div><strong>You request a demo</strong><small>Pick a context and enter your number.</small></div></li><li><span>2</span><div><strong>We call you in seconds</strong><small>No app, headset or setup required.</small></div></li><li><span>3</span><div><strong>You talk to HalaCX</strong><small>A real voice agent follows the selected scenario.</small></div></li><li><span>4</span><div><strong>You see the outcome</strong><small>The transcript and call details land in your workspace.</small></div></li></ol></div></div>
    </section>
