...
export default function MarketingScenes() {
  ...
  return <>
    ...
    <section className="live-demo-lab" id="live-demo">
      <div className="demo-form-side"><p>LIVE DEMO</p><h2>Don’t take our word for it.<br/>Take the call.</h2><span>Choose a conversation, choose a voice, then enter the number you want us to call.</span><div className="demo-flow"><p>THREE STEPS. ONE REAL CALL.</p><ol><li><span>1</span><div><strong>Choose the conversation</strong><small>Pick the job you want the agent to handle.</small></div></li><li><span>2</span><div><strong>Choose its voice</strong><small>Listen for the personality that fits your brand.</small></div></li><li><span>3</span><div><strong>Answer your phone</strong><small>We call in seconds. No app or setup required.</small></div></li></ol></div></div>
      <div className="demo-storyboard"><div className="context-tabs" role="tablist">{contexts.map(item=><button type="button" key={item.key} className={context.key===item.key?"active":""} onClick={()=>setContext(item)} role="tab" aria-selected={context.key===item.key}>{item.label}</button>)}</div><p className="context-line">{context.line}</p><div className="provider-picker"><header><label>CHOOSE THE VOICE</label><span>03 PERSONALITIES</span></header><div role="radiogroup" aria-label="Voice personality">{voiceProviders.map(provider=><button type="button" key={provider.key} data-provider={provider.key} className={voiceProvider===provider.key?"active":""} onClick={()=>setVoiceProvider(provider.key)} role="radio" aria-checked={voiceProvider===provider.key}><span className="provider-art" aria-hidden="true"><i className="provider-orbit"/><i className="provider-mark">{Array.from({length:5}).map((_,index)=><b key={index}/>)}</i><em>{provider.name.slice(0,1)}</em></span><span className="provider-copy"><i>{provider.badge}</i><strong>{provider.name}</strong><small>{provider.detail}</small><span><b>{provider.tone}</b><em>Select voice →</em></span></span></button>)}</div></div><form onSubmit={requestCall}><label htmlFor="demo-phone">YOUR MOBILE NUMBER</label><div><select aria-label="Country code" value={phone.startsWith("+91")&&!phone.startsWith("+971")?"+91":"+971"} onChange={event=>changePhoneCountry(event.target.value)}>{phoneCountries.map(country=><option key={country.code} value={country.code}>{country.label}</option>)}</select><input id="demo-phone" value={phone.replace(/^\+(?:971|91)/, "")} onChange={event=>changePhoneNumber(event.target.value)} inputMode="numeric" aria-label="Mobile number without country code" placeholder="Mobile number"/><button disabled={callState==="loading"}><Phone weight="fill"/>{callState==="loading"?"Calling…":"Call me"}</button></div>{message&&<small className={callState}>{message}</small>}</form></div>
    </section>
